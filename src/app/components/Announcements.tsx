const Announcements = () => {
  return (
    <div className="bg-white p-4 rounded-md">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Anúncios</h1>
        <span className="text-xs text-gray-400">Ver tudo</span>
      </div>
      <div className="flex flex-col gap-4 mt-4">
        <div className="bg-SkyLight rounded-md p-4">
          <div className="flex items-center justify-between">
            <h2 className="font-medium">Apresentação de Teatro</h2>
            <span className="text-xs text-gray-400 bg-white rounded-md px-1 py-1">
              2025-01-01
            </span>
          </div>
          <p className="text-sm text-gray-400 mt-1 line-clamp-2 overflow-hidden">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium
            iste ex enim quibusdam sapiente magnam dicta in architecto?
          </p>
        </div>
        <div className="bg-PurpleLight rounded-md p-4">
          <div className="flex items-center justify-between">
            <h2 className="font-medium">Apresentação de Teatro</h2>
            <span className="text-xs text-gray-400 bg-white rounded-md px-1 py-1">
              2025-01-01
            </span>
          </div>
          <p className="text-sm text-gray-400 mt-1 line-clamp-2 overflow-hidden">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium
            iste ex enim quibusdam sapiente magnam dicta in architecto?
          </p>
        </div>
        <div className="bg-YellowLight rounded-md p-4">
          <div className="flex items-center justify-between">
            <h2 className="font-medium">Apresentação de Teatro</h2>
            <span className="text-xs text-gray-400 bg-white rounded-md px-1 py-1">
              2025-01-01
            </span>
          </div>
          <p className="text-sm text-gray-400 mt-1 line-clamp-2 overflow-hidden">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium
            iste ex enim quibusdam sapiente magnam dicta in architecto?
          </p>
        </div>
      </div>
    </div>
  );
};

export default Announcements;
